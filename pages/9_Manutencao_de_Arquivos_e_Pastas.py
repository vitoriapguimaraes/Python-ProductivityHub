...
st.markdown("Visualize a hierarquia de diretórios ou gere listas de arquivos do seu sistema.")

# Criar as abas
tab_estrutura, tab_lista = st.tabs(["📁 Estrutura de Pastas", "📄 Listador de Arquivos"])
default_path = get_default_path()

# Aba 1: Visualizador de Estrutura
with tab_estrutura:
    st.header("Visualizador de Estrutura de Pastas")
    st.markdown("Exiba a hierarquia completa de arquivos e pastas a partir de um diretório inicial.")
    ...
    if st.button("Visualizar Estrutura 🔍", type="primary", key="btn_visualizar_estrutura"):
...
with tab_lista:
    ...
    st.markdown("Gera uma lista simples em texto com o nome de todos os arquivos contidos em uma pasta.")
    ...
    if st.button("Listar Arquivos 📝", type="primary", key="btn_listar_arquivos"):
        if not caminho_lista:
            ...
            if error:
                ...
                st.text_area("Conteúdo da Lista", report, height=300, key="txt_area_lister")
