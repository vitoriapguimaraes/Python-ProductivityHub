...
from core.image_tools import process_single_face_crop

st.set_page_config(page_title="Recortador de Rostos", page_icon="👤", layout="wide")
apply_global_style()
st.title("👤 Recortador de Rostos")
st.markdown(
    "Faça upload de fotos e nossa ferramenta irá detectar rostos, recortá-los e opcionalmente remover o fundo."
)
...
if remove_bg:
    st.sidebar.caption(
        "💡 Nota: A remoção de fundo com IA pode demorar alguns segundos por imagem na primeira execução."
    )
...
def _process_files(files, remove_bg):
    processed_images = []
    ...
    for idx, up_file in enumerate(files):
        with st.spinner(f"Processando {up_file.name}..."):
            try:
                # Chamar utilitário
                img_bytes, face_found = process_single_face_crop(
                    up_file, remove_bg=remove_bg
                )

                if not face_found:
                    st.warning(
                        f"⚠️ Rosto não detectado em {up_file.name}. Processando imagem inteira."
                    )
                ...
                processed_images.append(
                    {"name": out_name, "data": img_bytes, "detected": face_found}
                )
...
def _render_downloads_and_preview(processed_images):
...
if uploaded_files:
    if st.button("Iniciar Recorte 🚀", type="primary"):
        processed_images = _process_files(uploaded_files, remove_bg)
        if processed_images:
            _render_downloads_and_preview(processed_images)
