...
st.markdown("Transforme textos ou arquivos Markdown em documentos PDF elegantes usando Pandoc e LaTeX.")
...
with st.expander("Configurações do PDF", expanded=False):
    ...
    with c2:
        mainfont = st.selectbox("Fonte:", ["Arial", "Helvetica", "Times New Roman", "Courier"], index=0)
...
def convert_md_to_pdf_bytes(md_content, margin_val, font_name, add_page_numbers):
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix="_temp.md", mode='w', encoding='utf-8') as tmp_in:
    ...
    extra_args = [
        '--standalone',
        '--pdf-engine=xelatex',
        '-V', f'geometry:margin={margin_val}cm',
    ...
    if add_page_numbers:
        ...
        extra_args.extend([
            '-V', r'header-includes=\usepackage{lastpage}',
            '-V', r'header-includes=\usepackage{fancyhdr}',
            '-V', r'header-includes=\pagestyle{fancy}',
            '-V', r'header-includes=\fancyhf{}',
            '-V', r'header-includes=\renewcommand{\headrulewidth}{0pt}',
            '-V', fancyfoot_cmd,
        ])
    ...
    if st.button("Gerar PDF 🚀", key="btn_convert_text", type="primary"):
        if not md_text.strip():
            ...
            with st.spinner("Compilando PDF..."):
                try:
                    pdf_data = convert_md_to_pdf_bytes(md_text, margin, mainfont, number_pages)
...
with tab_arquivo:
    uploaded_file = st.file_uploader(
        "Escolha um arquivo Markdown (.md):",
        type=["md"],
        ...
        if st.button("Converter Arquivo 🚀", key="btn_convert_file", type="primary"):
            with st.spinner("Compilando PDF do arquivo..."):
                try:
                    ...
                    pdf_data = convert_md_to_pdf_bytes(content, margin, mainfont, number_pages)
