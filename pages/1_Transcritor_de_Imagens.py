...
st.set_page_config(page_title="Transcritor de Imagens (OCR)", page_icon="🔍", layout="wide")
apply_global_style()
st.title("🔍 Transcritor de Imagens (OCR Local)")
st.markdown("Extraia texto de fotos, documentos escaneados ou capturas de tela localmente de forma offline e gratuita usando EasyOCR.")
...
if "last_uploaded_files" not in st.session_state or st.session_state.last_uploaded_files != uploaded_filenames:
    st.session_state.ocr_results = {}
    st.session_state.last_uploaded_files = uploaded_filenames

if uploaded_files:
    # Mostrar preview das imagens se ainda não foram processadas
        ...
        for idx, uploaded_file in enumerate(uploaded_files):
            with st.spinner(f"Transcrevendo {uploaded_file.name}..."):
                try:
                    ...
                    st.session_state.ocr_results[uploaded_file.name] = f"Erro na transcrição: {e}"
            progress_bar.progress((idx + 1) / len(uploaded_files))
        progress_bar.empty()

    # Exibição dos resultados (se houver dados em ocr_results)
    if st.session_state.ocr_results:
        ...
        for idx, uploaded_file in enumerate(uploaded_files):
            ...
            with st.container(border=True):
                ...
                with col_txt:
                    st.markdown("**🔍 Texto Extraído (Monospace - Copie no botão superior direito):**")
                    st.code(text, language="text")
                    
                    # Campo editável para correções rápidas
        ...
        # Unificar os textos transcritos (considerando possíveis edições do usuário)
        all_texts = []
        ...
        # Opção de resumo e downloads
        if gerar_resumo:
            if not api_key:
                st.warning("⚠️ **API Key não configurada:** Para gerar resumos inteligentes automáticos via IA das suas imagens transcritas, configure a variável `OPENAI_API_KEY` no seu arquivo `.env`.")
                st.download_button(
                ...
                with st.spinner("🧠 Gerando Resumo Inteligente do texto extraído (OpenAI)..."):
                    try:
